...
void InspectorComponentRenderer::draw_script_component(const Origo::RID& entity, Origo::ScriptComponentInstance& instance) {
	...
	if (!draw_header_scripted(entity, instance, desc, label.c_str(), m_script_icon))
		return;
...
bool InspectorComponentRenderer::draw_header_scripted(const Origo::RID& entity, Origo::ScriptComponentInstance& instance, const Origo::ScriptComponentDescriptor& desc, const char* label, Origo::Ref<Origo::Texture2D> icon) {
	ImGui::PushID(&instance);

	bool open = false;

	if (ImGui::BeginTable("##script_inspector_row", 2, ImGuiTableFlags_SizingFixedFit)) {
		ImGui::TableSetupColumn("##icon", ImGuiTableColumnFlags_WidthFixed, 24.0f);
		ImGui::TableSetupColumn("##header", ImGuiTableColumnFlags_WidthStretch);
		ImGui::TableNextRow();
		...
		if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) && m_context)
			m_context->set_selected_asset(desc.ScriptID);
		...
		if (ImGui::ImageButton(
		        "##script_component_menu_btn",
		        menu_icon,
		        ImVec2(menu_size, menu_size))) {

			ImGui::OpenPopup("##script_component_menu");
		}

		ImGui::PopStyleColor(3);
		ImGui::PopStyleVar(2);

		if (ImGui::BeginPopup("##script_component_menu")) {
			if (ImGui::MenuItem("Open Script") && m_context)
				m_context->set_selected_asset(desc.ScriptID);

			if (ImGui::MenuItem("Remove Component") && m_context && m_context->ActiveScene) {
				m_context->ActiveScene->remove_script_component_if_exists(entity, instance.ID);
				open = false;
			}
