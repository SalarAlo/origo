...
static void DrawVec3Control(std::string_view label, glm::vec3& values, float speed = 0.1f) {
	...
	auto DrawField = [&](const char* id, float& v, const ImVec4& textColor) {
	...
	DrawField("X", values.x, ImVec4(0.85f, 0.35f, 0.35f, 1.0f));
	ImGui::SameLine(0.0f, spacingBetweenFields);
	DrawField("Y", values.y, ImVec4(0.45f, 0.85f, 0.45f, 1.0f));
	ImGui::SameLine(0.0f, spacingBetweenFields);
	DrawField("Z", values.z, ImVec4(0.35f, 0.55f, 0.95f, 1.0f));
...
static bool s_Registered = []() {
	ComponentDrawRegistry::Register<Origo::Transform>("Transform", [](Origo::Transform& t) {
