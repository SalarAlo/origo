...
{
    UpdateLayout();
    static const SpanInfo emptyLine {};
    if (m_windowLines.empty())
    {
        return emptyLine;
    }
...
NVec2i ZepWindow::BufferToDisplay(const GlyphIterator& loc)
{
    ...
    const auto* lastLine = m_windowLines[m_windowLines.size() - 1];
    if (lastLine->lineCodePoints.empty())
    {
        ret.x = 0;
        return ret;
    }
    ret.x = long(lastLine->lineCodePoints.size() - 1);
    return ret;
}
