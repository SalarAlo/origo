...
#include "origo/components/systems/SoundEmitterComponentSystem.h"
...
void SoundEmitterComponentSystem::update(Origo::Scene* scene, float dt) {
	...
	scene->view<SoundEmitterComponent>([&](RID entity, SoundEmitterComponent& sound_emitter) {
		...
		if (!audio) {
			ORG_WARN("SoundEmitterComponentSystem: audio handle is invalid or no longer points to an Audio asset");
			sound_emitter.WasEnabledLastFrame = sound_emitter.IsEnabled;
			return;
		}
...
REGISTER_UPDATE_SYSTEM(
    ...
    Origo::SoundEmitterComponentSystem);
}
