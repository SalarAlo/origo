...
glm::vec3 read_position(const std::vector<float>& vertices, size_t index, size_t stride) {
	const size_t offset = index * stride;
	return { vertices[offset + 0], vertices[offset + 1], vertices[offset + 2] };
}

void apply_mesh_bounds(AssetHandle handle, const MeshData& data, size_t stride) {
	if (data.Vertices.empty() || stride < 3)
		return;

	glm::vec3 min_bounds = read_position(data.Vertices, 0, stride);
	glm::vec3 max_bounds = min_bounds;

	for (size_t vertex_index = 1; vertex_index < data.Vertices.size() / stride; ++vertex_index) {
		const glm::vec3 position = read_position(data.Vertices, vertex_index, stride);
		min_bounds = glm::min(min_bounds, position);
		max_bounds = glm::max(max_bounds, position);
	}

	if (auto* mesh = AssetManager::get_instance().get_asset<Mesh>(handle))
		mesh->set_bounds(min_bounds, max_bounds);
}
...
AssetHandle PrimitiveShapeCache::get_cube_mesh() {
	static auto cube_handle = ([] {
		...
		auto cube_handle = AssetFactory::get_instance().create_synthetic_asset<Mesh>(
		    ...
		    heap_id,
		    range);
		apply_mesh_bounds(cube_handle, data, vertex_stride / sizeof(float));
...
AssetHandle PrimitiveShapeCache::get_wire_cube_mesh() {
	static auto wire_cube_handle = ([] {
		...
		auto wire_cube_handle = AssetFactory::get_instance().create_synthetic_asset<Mesh>(
		    ...
		    heap_id,
		    range);
		apply_mesh_bounds(wire_cube_handle, data, vertex_stride / sizeof(float));
...
AssetHandle PrimitiveShapeCache::get_wire_cone_mesh() {
	static auto wire_cone_handle = ([] {
		...
		auto wire_cone_handle = AssetFactory::get_instance().create_synthetic_asset<Mesh>(
		    ...
		    heap_id,
		    range);
		apply_mesh_bounds(wire_cone_handle, data, vertex_stride / sizeof(float));
...
AssetHandle PrimitiveShapeCache::get_cone_mesh() {
	static auto cone_handle = ([] {
		...
		auto cone_handle = AssetFactory::get_instance().create_synthetic_asset<Mesh>(
		    ...
		    heap_id,
		    range);
		apply_mesh_bounds(cone_handle, data, vertex_stride / sizeof(float));
