...
private:
	...
	bool is_import_necessary(const std::filesystem::path& path, const AssetMetadata& meta);
	void import_asset(const std::filesystem::path& path, IAssetImporter* importer, AssetMetadata& meta, int& importCount);
	void load_cached_asset(const std::filesystem::path& importFile, AssetMetadata& meta);
};
